...
def decline_guest(guest):
    """Function to tell people they're not invited after all"""
    message = "Sorry {}, you're not invited after all"
    print(message.format(guest.title()))
...
print("\nSeems like {} wasn't able to attend\n".format(declined_invitation.title()))
...
print_guest_list(invitation, guest_list)
# The table broke, must remove guests.
print('\nBig table broke, must let someone down\n')
# While loop to remove guests and tell them they're not invited
while(len(guest_list) > 2):
    decline_guest(guest_list.pop())
print_guest_list(invitation, guest_list)
